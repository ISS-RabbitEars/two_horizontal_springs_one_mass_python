...
def integrate(ic, ti, p):
	m, k1, k2, xeq1, xeq2, xp = p
	x, v = ic

	print(ti)

	return [v, A.subs({M:m, K1:k1, K2:k2, Xeq1:xeq1, Xeq2:xeq2, Xp:xp, X:x, Xdot:v})]


M, K1, K2, Xeq1, Xeq2, Xp, t = sp.symbols('M K1 K2 Xeq1 Xeq2 Xp t')
X = dynamicsymbols('X')

Xdot = X.diff(t, 1)

T = sp.Rational(1, 2) * M * Xdot**2
V = sp.Rational(1, 2) * (K1 * (X - Xeq1)**2 + K2 * (X - Xp + Xeq2)**2)
...
p = m, k1, k2, xeq1, xeq2, post2
ic = xo, vo

xv = odeint(integrate, ic, ta, args=(p,))

ke = np.asarray([T.subs({M:m, Xdot:i}) for i in xv[:,1]])
pe = np.asarray([V.subs({K1:k1, K2:k2, Xeq1:xeq1, Xeq2:xeq2, Xp:post2, X:i}) for i in xv[:,0]])
E = ke + pe

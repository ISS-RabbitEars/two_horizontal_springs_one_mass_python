...
def integrate(ic, ti, p):
	...
	return [v, A.subs({M:m, K1:k1, K2:k2, Xeq:xeq, X:x, Xdot:v})]


M, K1, K2, Xeq, t = sp.symbols('M K1 K2 Xeq t')
X = dynamicsymbols('X')

Xdot = X.diff(t, 1)

T = sp.Rational(1, 2) * M * Xdot**2
V = sp.Rational(1, 2) * (K1 * (X - Xeq)**2 + K2 * (X - Xeq)**2)
...
pe = np.asarray([V.subs({K1:k1, K2:k2, Xeq:xeq, X:i}) for i in xv[:,0]])
E = ke + pe
